...
- **`docker-compose`** — two `.env` variables switch registries: `WEBCORE_REGISTRY` for the proprietary webcore images and `COTS_REGISTRY` for the public ones. They are read differently, by one character, and it matters: `COTS_REGISTRY` uses `${COTS_REGISTRY-default}`, one dash, so the default applies only when the variable is **unset**. An empty value is its meaningful Internet setting, because “no prefix at all” is how an image reference reaches Docker Hub or a fully qualified `ghcr.io/…` name. `WEBCORE_REGISTRY` uses `:-`, where blank falls back to the default.
