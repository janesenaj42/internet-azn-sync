...
Gating differs by stack. The five MFE pipelines are advisory: `|| true` plus `allow_failure: true`, so findings land in artifacts and never fail a job. The eight backend pipelines gate already, with no `allow_failure` anywhere — Semgrep on any ERROR finding, Trivy on HIGH or CRITICAL in the jar, config tree, secret scan and built image. Each Trivy step runs twice, `--exit-code 0` for the artifact then `--exit-code 1` to gate, so evidence survives a failure.
...
- **`docker-compose`** — three `.env` variables switch registries: `WEBCORE_REGISTRY`, `COTS_REGISTRY` and `EJABBERD_REPO`. The first two are prefixes in front of a fixed image path, because `nginx` is `nginx` in both worlds. ejabberd is not: the Internet pulls `ghcr.io/processone/ejabberd:25.08` from the GitHub registry, while the air-gap mirror carries it as `ejabberd/ejabberd:25.08`. The whole `repo:tag` is therefore the variable, and ejabberd stays a third override rather than a prefix flip — change only the two registry variables and that one container fails to pull. A second trap sits beside it: `COTS_REGISTRY` is read as `${COTS_REGISTRY-default}`, one dash, which substitutes only when the variable is **unset**. An empty value must stay empty, which is also what lets the fully qualified `ghcr.io/…` name resolve unprefixed.
